...
class Integrator(threading.Thread):
    def __init__(self, X, B, update = None, maxiter = None):
    ...
    def run(self):
        ...
        while (self.run_key):
            self.program.Integrate(self.queue, self.shape, block_shape, 
                                   self.X, self.X1, self.B, self.DB, 
                                   np.float32(self.step), self.Error, self.Current, self.Bx)
            ...
            if (not np.isnan(error)):
                ...
                cl.enqueue_read_buffer(self.queue, self.Bx, self._Bx)
                self.step /= error**0.1+0.5
                if (error < 1):                 
                    cl.enqueue_read_buffer(self.queue, self.X1, self._X)
                    if (self.update != None):
                        self.update(self._X, self.time, self.step)
                    cl.enqueue_copy(self.queue, self.X, self._X.astype(np.float32))
                    self.time += self.step
            else:
                self.step /= 2.
            """
