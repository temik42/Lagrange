...
__kernel __attribute__((reqd_work_group_size(BLOCK_SIZE,BLOCK_SIZE,BLOCK_SIZE)))
void Integrate(__global float3* X, __global float3* X1, __global float3* B, __global float3* DB,
               float step, __global float* Error, __global float3* Current, __global float3* Bx)
{       
    __local float3 Xl[NL*NL*NL];
    ...
    for (i = 0; i < 6; i++) {     
        ...
        if (i == 0) Current[idx] = temp.current;
